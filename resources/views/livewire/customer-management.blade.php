<div>
                    ...
                    <div class="overflow-x-auto shadow-md sm:rounded-lg">
                        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                            <thead class="bg-zinc-50 dark:bg-zinc-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase tracking-wider">Nombre / Razón Social</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase tracking-wider">Documento</th>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase tracking-wider">Ubicación</th>
                                ...
                                @forelse ($customers as $customer)
                                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">{{ $customer->nombres_completos }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-zinc-900 dark:text-zinc-100 font-mono">{{ $customer->numero_documento }}</div>
                                            <div class="text-sm text-zinc-500 dark:text-zinc-400">{{ $customer->tipo_documento }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-zinc-900 dark:text-zinc-100">{{ $customer->telefono ?? '--' }}</div>
                                            <div class="text-sm text-zinc-500 dark:text-zinc-400">{{ $customer->correo_electronico ?? '--' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-zinc-900 dark:text-zinc-100">{{ $customer->ciudad }}</div>
                                            <div class="text-sm text-zinc-500 dark:text-zinc-400 truncate max-w-xs">{{ $customer->direccion_completa }}</div>
                                        </td>
    ...
    <x-crud-modal
        ...
        subtitle="Complete la información personal y de contacto"
        submit-action="saveCustomer"
        submit-text="Guardar Cliente"
    >
        <x-slot name="icon">
        ...
        <x-customer-form mode="create" prefix="create" />
    </x-crud-modal>
    ...
    >
        ...
        <x-customer-form mode="edit" prefix="edit" />
    </x-crud-modal>
    @endif
